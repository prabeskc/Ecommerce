...
exports.confirmUser=  async(req,res)=>{
  ...
  if(user.isVerified){
    return res.status(400).json({error:"already veriied,please login to continue"})
  }
  user.isVerified=true;
  await user.save();
  return res.status(200).json({message:"user verifed succesfully,please verify your email"})
}
...
exports.forgotPassword = async (req,res)=>{
  ...
  return res.status(200).json({message:"reset link has been sent to yout email"})
}
...
exports.resendConfirmation = async (req,res)=>{
  ...
  return res.status(200),json({message:"confirmaton link has been sent to your mail"})
}